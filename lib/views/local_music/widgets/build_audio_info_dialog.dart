...
buildAudioInfoDialog(BuildContext context, AudioLongPress alp) {
  ...
  return showDialog<void>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text('属性', style: TextStyle(fontSize: 20.sp)),
        content: StatefulBuilder(
          builder: (BuildContext context, StateSetter setState) {
            return SizedBox(
              width: double.maxFinite,
              height: list.length > 1 ? 200.sp : 250.sp,
              child: list.length > 1
                  ? ListView(
                      shrinkWrap: true,
                      itemExtent: 50.sp,
                  ...
                  : ListView(
                      ...
                      itemExtent: 50.sp,
                      children: <Widget>[
                        buildRowListTile("名称", list[0].displayName),
                        buildRowListTile("路径", list[0].data.toString()),
                        buildRowListTile(
                        ...
                        buildRowListTile("歌名", list[0].title),
                        buildRowListTile("歌手", list[0].artist ?? "未知歌手"),
                        buildRowListTile("专辑", list[0].album ?? "未知专辑"),
                        buildRowListTile(
                          "修改时间",
...
// 显示歌单信息的弹窗
buildAudioInfoDialogOld(BuildContext context, AudioLongPress alp) {
  // 获取当前被选中的音频列表
  List<SongModel> list = alp.selectedAudioList;
  // 获取多选音频时所有音频的路径地址，放到list中
  var localUrlList = list
